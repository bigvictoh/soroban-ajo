// Wallet type definitions for Freighter, Albedo, Lobstr, and Mobile Wallets

export type WalletType = 'freighter' | 'albedo' | 'lobstr' | 'freighter_mobile' | 'lobstr_mobile'
...
export interface AlbedoAPI {
  ...
  }) => Promise<{ pubkey: string; signed_message?: string }>
  tx: (params: {
    xdr: string
    network?: string
    submit?: boolean
...
// Deep link parameters from mobile wallet callbacks
...
// Mobile wallet info for UI display
export interface MobileWalletInfo {
  type: 'freighter_mobile' | 'lobstr_mobile'
  name: string
  isAvailable: boolean
